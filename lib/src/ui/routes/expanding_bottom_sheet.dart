...
import 'package:nour/src/ui/routes/shopping_cart.dart';
...
class ExpandingBottomSheet extends StatefulWidget {
  ...
  static _ExpandingBottomSheetState of(BuildContext context,
    ...
    assert(context != null);
    final _ExpandingBottomSheetState result = context.findAncestorStateOfType(
...
class _ProductThumbnailRowState extends State<ProductThumbnailRow> {
  ...
  void initState() {
    ...
    _list = _ListModel(
      listKey: _listKey,
      initialItems: Provider.of<AppStateModel>(context, listen: false)
          .productsInCart
          .keys
          .toList(),
...
class ExtraProductsNumber extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<AppStateModel>(
      builder: (BuildContext builder, AppStateModel model, Widget child) =>
